...
const GenerateBill = () => {
  ...
  const [column, setColumns] = useState([
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => <div style={{ width: "8rem" }}>{row.current}</div>,
    },
    {
  ...
  const getGenerateBillList = () => {
    getGeneratedBillAPI()
      .then((res) => {
        ...
        setFiltereddata(res.data.data); // Initialize filteredData with initial data
        }
        else {
  ...
  const handleSubmit = (formData) => {
    const { tower, month } = formData;
  
    // Trigger validation
    trigger().then((isValid) => {
      if (isValid) {
        // Filter data based on tower and month
        const filteredData = data.filter((item) => {
          return (
            item.tower === tower.value && // Assuming tower value is an object with 'value' property
            item.month === formatDate(month) // Assuming month is a Date object, and formatDate is a function to format it
          );
        });
  ...
  return (
    <>
      <div>
        ...
        <PagesDatatable columns={column} data={filtereddata} />
      </div>
